...
from syntex.agents.executor import executor_node
from syntex.agents.reviewer import reviewer_node

def route_executor(state: AgentState) -> str:
    # route back to coder if syntax fails, else go to reviewer
    if state.get("error"):
        return "coder"
    return "reviewer"
...
def create_agent_graph():
    ...
    # define conditional routing logic based on the executor's output
